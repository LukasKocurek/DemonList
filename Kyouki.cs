...
namespace DemonList
{
    public partial class Kyouki : Form
    {
        ...
        public Kyouki(string user)
        {
            InitializeComponent();
            currentUser = user;
        }

        private void Kyouki_Load(object sender, EventArgs e)
        {
        ...
        private void DemonListScoreNumber_Kyouki_Click(object sender, EventArgs e)
        {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            "</body></html>";
            var url = "https://www.youtube.com/embed/KDa5c0CJTHs"; // Použite odkaz na embed videa
            this.webBrowser1.DocumentText = string.Format(embed, url);

            // Skontrolujeme, či užívateľ už dokončil level pri načítaní formulára
            if (IsLevelBeaten(currentUser, "Kyouki"))
            {
                Complete_Kyouki.FillColor = Color.Green;
        ...
        private void BackButton_Kyouki_Click(object sender, EventArgs e)
        {
            new MainApp(currentUser).Show();
            this.Hide();
        }

        private void Complete_Kyouki_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                RecordScore(currentUser, "Kyouki", 253.53f); // Použijeme currentUser ako používateľské meno
                MessageBox.Show("Congratulations! You have completed Kyouki."); // Zobrazíme hlásenie, že užívateľ dokončil level
        ...
        private void LoadLeaderboardData()
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    adapter.Fill(table);
                    KyoukiGridView.DataSource = table;

                    KyoukiGridView.Columns["rank"].HeaderText = "Rank";
                    KyoukiGridView.Columns["username"].HeaderText = "Username";
                    KyoukiGridView.Columns["score"].HeaderText = "Score";
                }
            }
            catch (Exception ex)
            {
