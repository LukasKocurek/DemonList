...
namespace DemonList
{
    public partial class SakupenCircles : Form
    {
        string connectionString = "server=127.0.0.1;port=3306;username=root;password=;database=userinfo2";
        private string currentUser;

        public SakupenCircles()
        {
            InitializeComponent();
        }

        public SakupenCircles(string user)
        {
            InitializeComponent();
            currentUser = user;
        }

        private void SakupenCircles_Load(object sender, EventArgs e)
        {
            LoadLeaderboardData();
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            var embed = "<html><head>" +
                        ...
                        "</body></html>";
            var url = "https://www.youtube.com/embed/ofG2mJi9kEA"; // Použite odkaz na embed videa
            this.webBrowser1.DocumentText = string.Format(embed, url);

            // Skontrolujeme, či užívateľ už dokončil level pri načítaní formulára
            if (IsLevelBeaten(currentUser, "Sakupen Circles"))
            {
                Complete_SakupenCircles.FillColor = Color.Green;
                Leaderboard_header.Text = "Completed";
                Leaderboard_header.ForeColor = Color.Green;
            }
        }

        private void BackButton_SKC_Click(object sender, EventArgs e)
        {
            new MainApp(currentUser).Show();
            this.Hide();
        }

        private void Complete_SakupenCircles_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                RecordScore(currentUser, "Sakupen Circles", 221.47f); // Použijeme currentUser ako používateľské meno
                MessageBox.Show("Congratulations! You have completed Sakupen Circles."); // Zobrazíme hlásenie, že užívateľ dokončil level
        ...
        private void LoadLeaderboardData()
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    adapter.Fill(table);
                    SakupenCirclesGridView.DataSource = table;

                    SakupenCirclesGridView.Columns["rank"].HeaderText = "Rank";
                    SakupenCirclesGridView.Columns["username"].HeaderText = "Username";
                    SakupenCirclesGridView.Columns["score"].HeaderText = "Score";
                }
            }
            catch (Exception ex)
            {
        ...
        private void SakupenCirclesGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
