...
namespace DemonList
{
    public partial class SilentClubstep : Form
    {
        string connectionString = "server=127.0.0.1;port=3306;username=root;password=;database=userinfo2";
        private string currentUser;

        public SilentClubstep()
        {
            InitializeComponent();
        }

        public SilentClubstep(string user)
        {
            InitializeComponent();
            currentUser = user;
        }

        private void SilentClubstep_Load(object sender, EventArgs e)
        {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            "</body></html>";
            var url = "https://www.youtube.com/embed/GR4OMkS3SN8"; // Použite odkaz na embed videa
            this.webBrowser1.DocumentText = string.Format(embed, url);

            // Skontrolujeme, či užívateľ už dokončil level pri načítaní formulára
            if (IsLevelBeaten(currentUser, "Silent Clubstep"))
            {
                Complete_SilentClubstep.FillColor = Color.Green;
        ...
        private void SilentClubstepLabel_Click(object sender, EventArgs e)
        {

        }

        private void BackButton_SilentClubstep_Click(object sender, EventArgs e)
        {
            
        }

        private void Complete_SilentClubstep_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                RecordScore(currentUser, "Silent Clubstep", 291.70f); // Použijeme currentUser ako používateľské meno
                MessageBox.Show("Congratulations! You have completed Silent Clubstep."); // Zobrazíme hlásenie, že užívateľ dokončil level
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    string query = @"SELECT 
                                    ...
                                    FROM 
                                        leaderboard
                                    WHERE 
                                        level = 'Silent Clubstep';";

                    MySqlCommand cmd = new MySqlCommand(query, connection);
                    MySqlDataAdapter adapter = new MySqlDataAdapter(cmd);
                    DataTable table = new DataTable();
                    adapter.Fill(table);
                    SKCGridView.DataSource = table;

                    SKCGridView.Columns["rank"].HeaderText = "Rank";
                    SKCGridView.Columns["username"].HeaderText = "Username";
                    SKCGridView.Columns["score"].HeaderText = "Score";
                }
            }
            catch (Exception ex)
            {
        ...
        private void BackButton_SilentClubstep_Click_1(object sender, EventArgs e)
        {
